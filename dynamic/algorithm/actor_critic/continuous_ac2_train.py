...
if __name__ == '__main__':
    # Cart-Pole-v1 환경, 최대 타임스텝 수가 500
    gym.envs.register(
    ...
    for e in range(num_episode):
        ...
        while not done :
            ...
            if done :
                #에피소드마다 학습 결과 출력
                score_avg = 0.9 * score_avg + 1.0 * score if score_avg != 0 else score
                ...
                os.makedirs(os.path.join(BASEDIR, 'save_graph'), exist_ok=True)
                plt.savefig(os.path.join(BASEDIR, 'save_graph/graph_v2.png'))

                # 이동 평균이 400 이상일때 종료
                if score_avg > 400:
                    os.makedirs(os.path.join(BASEDIR, 'save_model'), exist_ok=True)
                    agent.model.save_weights(os.path.join(BASEDIR, 'save_model/model_v2'), save_format='tf')
                    sys.exit()
