...
class A3CAgent :
    def __init__(self, action_size, env_name):
        ...
        # 인공신경망을 업데이트하는 옵티마이저 함수 생성
        self.optimizer = AdamOptimizer(self.lr, use_locking=True)

        # 텐서보드 설정
        os.makedirs(os.path.join(BASEDIR, 'summary'), exist_ok=True)
        self.writer = tf.summary.create_file_writer(os.path.join(BASEDIR, 'summary', 'breakout_a3c'))
        # 학습된 글로벌신경망 모델을 저장할 경로 설정
        self.model_path = os.path.join(BASEDIR, 'save_model', 'breakout_model')
...
class Runner(threading.Thread):
    ...
    def draw_tensorboard(self, score, step, e):
        ...
        with self.writer.as_default():
            tf.summary.scalar('Total Reward/Episode', score, step=0)
            tf.summary.scalar('Average Max Prob/Episode', avg_p_max, step=0)
            tf.summary.scalar('Duration/Episode', step, step=0)
    ...
    def compute_loss(self,done):
        ...
        # 정책 신경망 업데이트
        action = tf.convert_to_tensor(self.actions, dtype=np.float32)
    ...
    def train_model(self, done):
        ...
        # 로컬 신경망의 오류함수를 줄이는 방향으로 글로벌신경망을 업데이트
        self.optimizer.apply_gradients(zip(grads, global_params))
        # 로컬 신경망의 가중치를 글로벌신경망의 가중치로 업데이트
    ...
    def run(self):
        ...
        while episode < num_episode:
            ...
            history = np.tile(state[np.newaxis, :, :, np.newaxis], 4)
